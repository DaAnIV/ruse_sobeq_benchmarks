...
class ChildIterator(
  ...
  def iterate(): Boolean = {
    ...
    while (!success && this.iter.hasNext) {
      val prog = iter.next()
      Condition.makeValid((pre, post), (prog.preCondition, prog.postCondition)) match {
        case Some((pre, post)) =>
          // success
...
class ChildrenIterator(
  ...
  resetItersForward(0, Condition.emp(this.variables), Condition.emp(this.variables))
  // -------------------------------------------------------------------------------------
  ...
  def tryIterateForward(i: Int): Boolean = {
    ...
    while (!success && iter.iterate() && this.deadline.hasTimeLeft()) {
      if (i == this.childCount - 1) {
        ...
        if (this.resetItersForward(i + 1, pre, post)) {
  ...
  def resetItersForward(
    ...
    while (!resetItersForward(i + 1, newPre, newPost)) {
  ...
  def makeProgramIter(postCondition: PostCondition, typ: Types.Type): Iterator[Program[_]] = {
    this.previousLevelChildren.flatMap { case (preCondition: Condition, programs: mutable.Buffer[Program[_]]) =>
      if (preCondition.maybeSatisfiableBy(postCondition)) {
        programs.filter(p_and_i => p_and_i.typ == typ)
      } else {
        Nil
      }
    }.iterator
