  ...
  def printResults(benchmark: Path, task: SynthesisTask, results: List[SynthesisResult]): Unit = {
    if (!this.headerPrinted) {
      ...
      try {
        this.csvWriter.println(
          "name,time (ms),ast size (raw),ast size (postprocessed),correct,store type,store size,subsumed by discarding,pruned by OE pure,pruned by OE mut,solution,ismut"
        )
      } catch {
    ...
    results.map(toBenchmarkResults(name, task, _)).foreach {
      case rs @ BenchmarkResult(
        ...
        print(" ")
        print(storeType)
        print(" ".repeat(20 - storeType.length))
        print(name)
        ...
        this.csvWriter
          .println(
            s"\"$name\",$time,$rawSizeStr,$postprocessedSizeStr,$correctStr,$storeType,$storeSize,$subsumedByDiscarding,$prunedByOEPure,$prunedOEMut,\"$solution\",$mutStr"
          )
...
object BenchmarkRunner extends App {
  ...
  if (config.testOE) {
    for (oe <- List(Store.SubsumptionCounts(), Store.PureOE(), Store.NoOE())) {
      Store.storeType = oe
