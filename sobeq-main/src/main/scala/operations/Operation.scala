...
object VariableOperation {
  def makeVariable[T](name: String, idx: Int, typ: Types.Type, values: Seq[T], variables: Int): Program[T] = {
    val op = typ match {
    ...
    op(Condition.emp(variables).mutate(idx, values)).asInstanceOf[Program[T]]
  }
}
...
abstract class VariableOperation[T](val varname: String, val varIdx: Int) extends LeafOperation {
  def apply(cond: Condition): Program[T] = {
    ...
    val thisCond = cond.toVariable(varIdx)
    val digger = this.makeDigger(Nil)
    Program(
      this,
      cond(varIdx).get.asInstanceOf[List[T]],
      Nil,
      thisCond,
      thisCond,
      digger
