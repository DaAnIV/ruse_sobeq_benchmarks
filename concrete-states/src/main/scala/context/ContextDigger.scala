...
trait VarDigger[T] extends ContextDigger {
  ...
  override def get(ctx: Condition): Seq[T] = ctx(varIdx).asInstanceOf[List[T]]
  ...
  override def update(
    ...
    val currentVals = preCondition(this.varIdx)
    val returnValues = retVal(currentVals)
    val mutatedValues = newVal(currentVals)
    val postCondition = preCondition.mutate(this.varIdx, mutatedValues)
