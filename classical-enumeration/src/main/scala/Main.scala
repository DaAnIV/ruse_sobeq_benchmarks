...
object Main extends App {
  def doSynthesize(task: SynthesisTask, timeout: Int = 30, debug: Boolean = true, bothSolutions: Boolean = false): List[SynthesisResult] = {
    val vocab = Vocabulary.immutVocabFromTask(task)
  ...
  def doSynthesize(
    ...
    // This is the classical OE Pure components branch
    // if a benchmark requires mutating the state, we can fail immediately
